...
for c1 = 1:length(mRNA_names)
    ...
    for c2 = 1:size(design_data,1)
        ...
        if start_codon_index == 0
            ...
            fprintf('***** SeriesA %s ERROR: Suitable start codon not found in design %d. *****\n',mRNA_names{c1},c2);
            continue;
        end
        aa_seq = nt2aa(probe_seq(start_codon_index:end));
        if ~isempty(findstr(aa_seq,'*'))
            design_spec_data(c2,1) = 0;
            fprintf('SeriesA %s: Design %d of %d has in-frame stop codon. Continuing.\n',mRNA_names{c1},c2,size(design_data,1));
            continue;
        end
    end
    %variable sets:
    ...
    %nanum(:,2)*2+nanum(:,3)*1+nanum(:,5)*3: Zika scoring function
    design_score_original = (design_spec_data(:,2)*3 + design_spec_data(:,4)*4 + design_spec_data(:,6)*5).*(design_spec_data(:,1) ~= 0) + (design_spec_data(:,1) == 0)*1e6;
    [sorted_design_score_original,sorted_indices1] = sort(design_score_original);
    
    %take scoring function used for original toehold switch paper 
    design_score = (norm_design_spec_data(:,2)*3 + norm_design_spec_data(:,4)*4 + norm_design_spec_data(:,6)*5).*(design_spec_data(:,1) ~= 0) + (design_spec_data(:,1) == 0)*1e6;
    [sorted_design_score,sorted_indices2] = sort(design_score);
    combined_data = [];
    labels = {'Score with Normalization','Score with No Normalization','Rank','Position','Probe Defect','Probe Min. Defect','Toehold Defect','Target Defect','Base mRNA Defect','Active mRNA Defect','DeltaG Active mRNA','Probe/mRNA Binding Defect','Probe/mRNA Active Translation Defect','Binding Fraction',...
    ...
    while 1
        ...
        if size(best_peaks,1) < num_designs%4
            ...
            if discard_window1 <= 0
                temp_max = size(design_score,1);
                [temp_sorted,temp_indices] = sort(design_score);
