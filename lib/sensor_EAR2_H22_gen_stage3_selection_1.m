...
for c1 = 1:length(mRNA_groups)
    ...
    [sorted_design_scores1_mean,indices_mean1] = sort(design_scores1(:,1),'Descend');
    [~,indices_min1] = sort(design_scores1(:,2),'Descend');
    [~,indices_max1] = sort(design_scores1(:,3),'Descend');
    [~,indices_mean2] = sort(design_scores2(:,1),'Descend');
    [~,indices_min2] = sort(design_scores2(:,2),'Descend');
    [~,indices_max2] = sort(design_scores2(:,3),'Descend');
...
for c1 = 1:size(sel_designs,1)
    ...
    for c2 = 1:round(size(sel_designs,2)/4)
        ...
        for c3 = 1:size(temp_table,1)
            ...
            output_table(end+1,1:3) = {sprintf('EAR_H22_%s_sens%s',subtarget_name,char('A'-1+c3)),sensor_seq,dna2rna_B(temp_table{c3,2})};
            sensor_RNAs(end+1,:) = {sprintf('EAR_H22_%s_sens%s',subtarget_name,char('A'-1+c3)),sensor_seq};
            short_target_RNAs(end+1,:) = {sprintf('EAR_H22_%s_targ%s',subtarget_name,char('A'-1+c3)),dna2rna_B(concatDNA('',temp_table{c3,2}))};
        end
        output_table{end+1,1} = '';
    end
end    
...
cd final_designs;
writeCSVFilefromStructure('EAR_H22_final_design_info.csv',output_table);
target_DNAs = {};
...
writeCSVFilefromStructure('EAR_H22_target_DNA_sequences.csv',target_DNAs);
writeCSVFilefromStructure('EAR_H22_sensor_DNA_sequences.csv',sensor_DNAs);
writeCSVFilefromStructure('EAR_H22_subtarget_RNA_sequences.csv',short_target_RNAs);
cd ../
