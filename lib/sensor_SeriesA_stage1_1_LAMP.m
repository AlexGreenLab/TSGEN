...
for c1 = 1:length(mRNA_names0)
    T_set = eval(input_table{c1,4});
    curr_output_name = input_table{c1,5};
    curr_output_seq = dna2rna_B(input_table{c1,6}(1:min(end,hairpin_suffix_addon_len)));
...
len_a = 3;
len_b = 3;
len_c = 26;%toehold region shorten for LAMP_loop
len_d = length(d_dom);
counter = 1;
