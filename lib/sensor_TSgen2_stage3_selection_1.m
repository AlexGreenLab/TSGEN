...
for c1 = 1:size(sel_designs,1)
    ...
    for c2 = 1:round(size(sel_designs,2)/4)
        ...
        for c3 = 1:size(temp_table,1)
            ...
            output_table(end+1,1:3) = {sprintf('TSgen2_%s_sens%s',subtarget_name,char('A'-1+c3)),sensor_seq,dna2rna_B(temp_table{c3,2})};
            sensor_RNAs(end+1,:) = {sprintf('TSgen2_%s_sens%s',subtarget_name,char('A'-1+c3)),sensor_seq};
            short_target_RNAs(end+1,:) = {sprintf('TSgen2_%s_targ%s',subtarget_name,char('A'-1+c3)),dna2rna_B(concatDNA('',temp_table{c3,2}))};
        end
        output_table{end+1,1} = '';
        counterx = counterx + 1;
    end
...
cd final_designs;
writeCSVFilefromStructure('TSgen2_final_design_info.csv',output_table);
target_DNAs = {};
...
writeCSVFilefromStructure('TSgen2_target_DNA_sequences.csv',target_DNAs);
writeCSVFilefromStructure('TSgen2_sensor_DNA_sequences.csv',sensor_DNAs);
writeCSVFilefromStructure('TSgen2_subtarget_RNA_sequences.csv',short_target_RNAs);
cd ../
